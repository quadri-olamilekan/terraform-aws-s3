...
variable "aws_iam_role_name" {
  ...
  description = "(Optional, Forces new resource) Friendly name of the iam role for the replication."
}

variable "aws_iam_role_policy_attachment_name" {
  type        = string
  default     = "tf-iam-role-policy-replication-12345"
  description = "(Optional, Forces new resource) Friendly name of the iam role policy attachment for the replication."
}
...
variable "rep_rule_id" {
  type        = string
  default     = "foobar"
  description = "(Optional) Unique identifier for the rule for Replication. Must be less than or equal to 255 characters in length."
}
...
variable "rep_rule_status" {
  type        = string
  default     = "Enabled"
  description = "(Required) Status of the rule for Replication. Either Enabled or Disabled The rule is ignored if status is not Enabled"
}

variable "replica_modifications_status" {
  type        = string
  default     = "Enabled"
  description = "(Required) Whether the existing objects should be replicated. Either Enabled or Disabled"
}
...
variable "replication_time_status" {
  type        = string
  default     = "Enabled"
  description = "(Required) Status of the Replication Time Control. Either Enabled or Disabled"
...
variable "replication_time" {
  type        = number
  default     = 15
  description = "(Required) Configuration block specifying the time by which replication should be complete for all objects and operations on objects."
}
...
variable "metrics_status" {
  type        = string
  default     = "Enabled"
  description = "(Required) Status of the Destination Metrics. Either Enabled or Disabled"
}

variable "metrics_time" {
  type        = number
  default     = 15
  description = "(Required) Time in minutes. Valid values: 15"
}

variable "delete_marker_replication_status" {
  type        = string
  default     = "Enabled"
  description = "status - (Required) Whether delete markers should be replicated. Either Enabled or Disabled"
}

variable "lc_rule_id_b" {
  type        = string
  default     = "backend-rule"
  description = "(Optional) Unique identifier for the rule for destination bucket lifecycle configuration. Must be less than or equal to 255 characters in length."
}

variable "lc_rule_status_b" {
  type        = string
  default     = "Enabled"
  description = "(Required) Status of the rule for destination bucket lifecycle configuration. Either Enabled or Disabled The rule is ignored if status is not Enabled"
}
...
variable "exp_days_b" {
  type        = number
  default     = 365
  description = "(Optional) Lifetime, in days, of the objects that are subject to the rule. The value must be a non-zero positive integer."
